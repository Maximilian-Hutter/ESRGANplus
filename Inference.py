...
parser.add_argument('--modelpath', type=str, default='weights/2x_ESRGANplusF7_epoch_99.pth', help=("path to the model .pth files"))
parser.add_argument('--inferencepath', type=str, default='../../data/InferenceImages/8023.jpg', help=('path to the inference images'))
...
PATH = opt.modelpath
imagepath = opt.inferencepath
image = Image.open(imagepath)
data_transforms = transforms.Compose([transforms.Resize((opt.hr_height // 4, opt.hr_width // 4)), transforms.ToTensor()])
...
model=ESRGANplus(opt.channels, filters=opt.filters,hr_shape=hr_shape, n_resblock = opt.n_resblock, num_upsample = opt.upsample)
model.load_state_dict(torch.load(PATH,map_location=('cpu')))
model.eval()

out = model(image)
utils.save_image(out,'../../results/ESRGANplus/HR_image.jpg')